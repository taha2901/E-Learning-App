...
class _CourseDetailsScreenState extends State<CourseDetailsScreen> {
  ...
  void initState() {
    ...
    _loadWatchedVideos();
    if (_userId.isNotEmpty) WishlistNotifier().load(_userId);
  }

  Future<void> _loadWatchedVideos() async {
    if (_userId.isEmpty) return;
    final ids = await CoursesRepo().fetchWatchedVideoIds(_userId, widget.course.id);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          _CourseHeroAppBar(course: widget.course, userId: _userId),
          SliverToBoxAdapter(child: _CourseInfoSection(course: widget.course)),
          if (_isEnrolled && total > 0)
            SliverToBoxAdapter(
              child: Padding(
                padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
                child: _ProgressSection(progress: progress, watched: watched, total: total),
              ),
            ),
          SliverToBoxAdapter(
            child: _DetailsTabs(
...
class _CourseHeroAppBar extends StatelessWidget {
  final CourseModel course;
  final String userId;
  const _CourseHeroAppBar({required this.course, required this.userId});
  ...
  Widget build(BuildContext context) {
    return SliverAppBar(
      ...
      actions: [
        // ✅ Bookmark Button
        AnimatedBuilder(
          animation: WishlistNotifier(),
          builder: (ctx, _) {
            final saved = WishlistNotifier().isSaved(course.id);
            return GestureDetector(
              onTap: () async {
                if (userId.isEmpty) return;
                await WishlistNotifier().toggle(userId, course);
                if (ctx.mounted) {
                  ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
                    content: Text(WishlistNotifier().isSaved(course.id)
                        ? 'Saved to wishlist ❤️'
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: saved ? Colors.white.withOpacity(0.25) : Colors.black.withOpacity(0.3),
...
class _ProgressSection extends StatelessWidget {
  ...
  final int watched;
  final int total;
  const _ProgressSection({required this.progress, required this.watched, required this.total});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: done ? AppColors.success.withOpacity(0.08) : AppColors.primary.withOpacity(0.06),
      ...
      child: Column(
        children: [
          Row(
            children: [
              Icon(done ? Icons.check_circle_rounded : Icons.play_lesson_outlined,
                  color: done ? AppColors.success : AppColors.primary, size: 18),
              const SizedBox(width: 8),
              Text(done ? 'Course Completed! 🎉' : '$watched of $total lessons watched',
        ...
        children: [
          Text(course.category.toUpperCase(),
              style: AppTextStyles.caption.copyWith(
                  color: AppColors.primary, fontWeight: FontWeight.w700, letterSpacing: 0.8)),
          ...
          Row(
            children: [
              const CircleAvatar(radius: 16,
                  backgroundImage: NetworkImage('https://picsum.photos/seed/instructor/100/100')),
              const SizedBox(width: 8),
          ...
          Wrap(spacing: 12, runSpacing: 8, children: [
            _InfoChip(icon: Icons.star_rounded, label: '${course.rating}', color: const Color(0xFFFBBF24)),
            _InfoChip(icon: Icons.people_outline_rounded, label: '${_fmt(course.studentsCount)} students'),
            _InfoChip(icon: Icons.play_lesson_outlined,
                label: '${course.videos.isNotEmpty ? course.videos.length : course.lessonsCount} lessons'),
            _InfoChip(icon: Icons.schedule_outlined, label: course.duration),
            _InfoChip(icon: Icons.signal_cellular_alt_rounded, label: course.level),
          ]),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(children: [
          _Tab(label: 'About', isSelected: selectedTab == 0, onTap: () => onTabChanged(0)),
          _Tab(label: 'Lessons', isSelected: selectedTab == 1, onTap: () => onTabChanged(1)),
        ]),
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(course.description, style: AppTextStyles.bodyLarge.copyWith(height: 1.7)),
  ...
  Widget build(BuildContext context) {
    if (course.videos.isEmpty) {
      return const Padding(
        ...
        child: Center(child: Column(children: [
          Icon(Icons.video_library_outlined, size: 64, color: AppColors.textHint),
          SizedBox(height: 16),
          Text('No lessons available yet', style: TextStyle(color: AppColors.textSecondary)),
        ])),
      );
    }
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${course.videos.length} Lessons  •  ${course.duration}', style: AppTextStyles.bodyMedium),
          const SizedBox(height: 16),
          ...course.videos.map((video) => _LessonItem(
            video: video,
            isEnrolled: isEnrolled,
            isWatched: watchedVideoIds.contains(video.id),
            onTap: () { if (!video.isLocked || isEnrolled) onVideoTap(video); },
          )),
        ],
      ),
    );
...
class _LessonItem extends StatelessWidget {
  ...
  const _LessonItem({required this.video, required this.isEnrolled, required this.isWatched, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(AppConstants.radiusL),
          border: Border.all(
              color: isWatched ? AppColors.success.withOpacity(0.3) : AppColors.cardBorder),
        ),
        child: Row(
          children: [
            Container(
              width: 42, height: 42,
              decoration: BoxDecoration(
                color: isWatched ? AppColors.success.withOpacity(0.1)
                    : locked ? AppColors.surfaceVariant : AppColors.primary.withOpacity(0.1),
                shape: BoxShape.circle,
              ),
              child: Icon(
                isWatched ? Icons.check_rounded : locked ? Icons.lock_outline_rounded : Icons.play_arrow_rounded,
                color: isWatched ? AppColors.success : locked ? AppColors.textHint : AppColors.primary,
                size: 20,
              ),
            ),
...
class _BottomEnrollBar extends StatelessWidget {
  ...
  const _BottomEnrollBar({required this.isEnrolled, required this.onEnroll, required this.onContinue});
      ...
      child: SafeArea(
        child: isEnrolled
            ? Row(children: [
                ...
                Text('You are enrolled!', style: AppTextStyles.h3.copyWith(color: AppColors.success)),
                const Spacer(),
                ElevatedButton(
            ...
            : AppPrimaryButton(label: 'Enroll Now – Free', onTap: onEnroll),
      ),
    );
  }
}